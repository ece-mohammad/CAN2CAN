...
HAL_StatusTypeDef bxCAN_SetFilterPolicy(uint16_t filter_id, uint32_t filter_mask) {
  ...
  filter.FilterMaskIdHigh = ((filter_mask) & 0x700) >> 8;
  filter.FilterMaskIdLow = (filter_mask * 0xFF) << 5;
  filter.FilterIdHigh = (filter_id & 0x700) >> 8;
  filter.FilterIdLow = (filter_id & 0xFF) << 5;
