...
/* timer stack memory */
static StackType_t timerTaskStackMemory [configTIMER_TASK_STACK_DEPTH] = {0};
static StaticTask_t timerTaskBuffer = {0};
...
void vApplicationGetTimerTaskMemory(StaticTask_t **ppxTimerTaskTCBBuffer, StackType_t **ppxTimerTaskStackBuffer, uint32_t *pulTimerTaskStackSize) {
  uint32_t timer_task_stack_size = configTIMER_TASK_STACK_DEPTH;

  (*ppxTimerTaskTCBBuffer) = &timerTaskBuffer;
  (*ppxTimerTaskStackBuffer) = timerTaskStackMemory;
  (*pulTimerTaskStackSize) = timer_task_stack_size;
}

__weak void configureTimerForRunTimeStats(void) {}

__weak unsigned long getRunTimeCounterValue(void) { return HAL_GetTick(); }
/* USER CODE END 1 */
